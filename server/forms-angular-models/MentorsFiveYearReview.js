...
var MentorsFiveYearReviewSchema = new Schema({
...
MentorsFiveYearReviewSchema.pre('save', function (next) {
...
var MentorsFiveYearReview;
try {
  MentorsFiveYearReview = mongoose.model('MentorsFiveYearReview');
} catch (e) {
  MentorsFiveYearReview = mongoose.model('MentorsFiveYearReview', MentorsFiveYearReviewSchema);
}

// Alternative form schemas can be defined as shown below
MentorsFiveYearReviewSchema.statics.form = function (layout) {
  var formSchema = '';
  switch (layout) {
...
MentorsFiveYearReviewSchema.statics.findAccepted = function (req, cb) {
  // Only show the accepted items
  cb(null, {accepted: true});
};

MentorsFiveYearReviewSchema.statics.prepareSave = function (doc, req, cb) {
  doc.ipAddress = req.ip;
  cb(null);
};

MentorsFiveYearReviewSchema.statics.report = function (report) {
  var reportSchema = '';
  switch (report) {
    case 'allVisible' :
      reportSchema = {
...
module.exports = {
  model: MentorsFiveYearReview,                                          // pass the model in an object if you want to add options
  options: {
    findFunc: MentorsFiveYearReviewSchema.statics.findAccepted,            // this can be used to 'pre' filter selections.
    ...
    onSave: MentorsFiveYearReviewSchema.statics.prepareSave                // a hook that can be used to add something from environment to record before update
  }
};